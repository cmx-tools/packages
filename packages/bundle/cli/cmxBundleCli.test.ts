...
import { CMX_BUNDLE_FILE_NAME, parseCmxBundleJson } from "@cmx-tools/contracts";
  ...
  it("supports omitted compile verb", async () => {
    await withTempDir(async (tempDir) => {
      ...
      await writeFixture(
        tempDir,
        "cmx.config.ts",
        "export default { exports: { default: { required: true, type: { from: '@cmx-tools/contracts', import: 'CmxNode' } } } };\n",
      );
  ...
  it("consumes value flags without positional corruption", async () => {
    await withTempDir(async (tempDir) => {
      ...
      await writeFixture(
        tempDir,
        "cmx.config.ts",
        "export default { exports: { default: { required: true, type: { from: '@cmx-tools/contracts', import: 'CmxNode' } } }, externals: ['@pkg/config'] };\n",
      );
