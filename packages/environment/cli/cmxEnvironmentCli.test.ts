    ...
    await withTempDir(async (tempDir) => {
      ...
      await writeFixture(
        ...
        [
          "export default {",
          "  exports: {",
          "    default: { required: true, type: { from: '@cmx-tools/contracts', import: 'CmxNode' } }",
          "  },",
          "  externals: [",
  ...
  it("supports omitted generate verb", async () => {
    await withTempDir(async (tempDir) => {
      ...
      await writeFixture(
        tempDir,
        "cmx.config.ts",
        "export default { exports: { default: { required: true, type: { from: '@cmx-tools/contracts', import: 'CmxNode' } } }, externals: ['@site/contract'] };\n",
      );
  ...
  it("writes diagnostics to stderr and exits 1 on failure", async () => {
    await withTempDir(async (tempDir) => {
      ...
      await writeFixture(
        tempDir,
        "cmx.config.ts",
        "export default { exports: { default: { required: true, type: { from: '@cmx-tools/contracts', import: 'CmxNode' } } }, externals: ['@site/missing'] };\n",
      );
